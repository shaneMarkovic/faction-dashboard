// ==UserScript==
// @name         Torn Ops — War Enforcer
// @namespace    torn-faction-dashboard
// @version      1.0.0
// @description  Disables attack links during a faction war when caps are reached; reads rules/progress from the Torn Ops dashboard backend. Rules are edited in the dashboard, not in-game.
// @author       Torn Ops (DOM engine adapted from Torn War Enforcer by KamiRen [2805199], MIT)
// @license      MIT
...
// @grant        GM_setValue
// @grant        GM_getValue
// @connect      yqaafrmptxjhbnjybqni.supabase.co
// ==/UserScript==
...
(function () {
  ...
  if (document.getElementById('tw-singleton')) return; // page-level singleton
  { const m = document.createElement('meta'); m.id = 'tw-singleton'; (document.head || document.documentElement).appendChild(m); }

  // ──────────────── CONFIG ────────────────
  const SUPABASE_URL = 'https://yqaafrmptxjhbnjybqni.supabase.co';
  const ANON_KEY = 'sb_publishable_NmtrdL67iYTGByWm2OdjrQ_oN5GeLP8';
  // Set this to YOUR faction id. Multi-faction: each faction distributes the
  // script with its own FACTION_ID (the same dashboard backend serves all).
  const FACTION_ID = 34247;
  const POLL_MS = 10000;
  const ENFORCE_ACTIVITY_RULE = true;
  // ─────────────────────────────────────────
  ...
  let ENEMY_TS_BY_UID = {}, ENEMY_IDS = null, IDLE_TARGET_MIN = 10;
  let LAST_STATE = null, WAR_ACTIVE = false;
  ...
  document.addEventListener('click', (e) => {
    ...
    if (a) {
      const reasons = (a.getAttribute('data-tw-reasons') || '').split('\n').filter(Boolean);
      if (reasons.length) { showTip(reasons, e); clearTimeout(tipTimer); tipTimer = setTimeout(hideTip, 3500); }
    } else { hideTip(); }
  }, true);
  ...
  function setBlocked(el, on, reasons) {
    if (on) {
      el.setAttribute('data-tw-reasons', (reasons || []).join('\n'));
      if (el.getAttribute('title')) { if (!el.hasAttribute('data-tw-title')) el.setAttribute('data-tw-title', el.getAttribute('title')); el.removeAttribute('title'); }
      if (el.getAttribute('aria-label')) { if (!el.hasAttribute('data-tw-aria')) el.setAttribute('data-tw-aria', el.getAttribute('aria-label')); el.removeAttribute('aria-label'); }
      ...
      if (isIcon) { el.style.outline = '1px solid #ff5252'; el.style.outlineOffset = '1px'; el.style.borderRadius = '3px'; }
      ...
      el.style.outlineOffset = ''; el.style.borderRadius = ''; el.style.cursor = '';
  ...
  function linkUid(el) { const m = (el.getAttribute('href') || '').match(/user2ID=(\d+)/i); return m ? m[1] : null; }

  function reasonsFor(el) {
    const onFaction = routeActive();
    const id = linkUid(el);
    if (!onFaction) { if (!ENEMY_IDS || !id || !ENEMY_IDS.has(id)) return []; }
    const r = BLOCK ? REASONS.slice() : [];
    if (WAR_ACTIVE && ENFORCE_ACTIVITY_RULE && IDLE_TARGET_MIN > 0) {
      const dot = id ? STATUS_BY_UID[id] : null;
      const ts = id ? ENEMY_TS_BY_UID[id] : null;
      if (dot === 'online') { r.push('Target is online — do not attack'); }
      else if (dot === 'idle') {
  ...
  // ---- panel ----
  let panel, statusEl, idEl, headerEl, headTitleEl, bodyEl, minBtn, refreshBtn;
  let minimized = false, disabled = false;
  ...
  function mkCtrlBtn(txt, title) {
    ...
    b.style.cssText = 'background:#222;color:#ddd;border:1px solid #3a3a3a;border-radius:6px;width:28px;height:28px;line-height:1;text-align:center;cursor:pointer;font-size:16px;padding:0;margin-left:5px;user-select:none;touch-action:manipulation';
    return b;
  }
  function spin(btn) { btn.style.transition = 'transform .5s'; btn.style.transform = 'rotate(360deg)'; setTimeout(() => { btn.style.transition = ''; btn.style.transform = ''; }, 520); }
  function applyPanelPosition() {
    if (!panel) return;
    if (minimized) { panel.style.left = 'auto'; panel.style.top = 'auto'; panel.style.right = MIN_DOCK.right; panel.style.bottom = MIN_DOCK.bottom; return; }
    let p = null; try { p = JSON.parse(pload('tw_pos') || 'null'); } catch (_) {}
    if (p && p.left && p.top) { panel.style.left = p.left; panel.style.top = p.top; panel.style.right = 'auto'; panel.style.bottom = 'auto'; }
    else { panel.style.left = 'auto'; panel.style.top = 'auto'; panel.style.right = '12px'; panel.style.bottom = '12px'; }
  }
  ...
  function ensurePanel() {
    ...
    panel.style.cssText = ['position:fixed','right:12px','bottom:12px','z-index:99999','cursor:move',"font:12px/1.45 -apple-system,'Segoe UI',sans-serif",'background:#181818','color:#f0f0f0','border:1px solid #2a2a2a','border-radius:10px','padding:9px 12px','min-width:200px','box-shadow:0 4px 16px rgba(0,0,0,.45)','user-select:none'].join(';');
    headerEl = document.createElement('div');
    headerEl.style.cssText = 'display:flex;align-items:center;justify-content:space-between;gap:10px;cursor:move';
    headTitleEl = document.createElement('div'); headTitleEl.style.cssText = 'white-space:nowrap;font-size:13px';
    setHeadTitle('', '#22c48a');
    const controls = document.createElement('div'); controls.style.cssText = 'display:flex;flex:0 0 auto';
    refreshBtn = mkCtrlBtn('⟳', 'Refresh'); minBtn = mkCtrlBtn('–', 'Minimize');
    ...
    statusEl = document.createElement('div'); statusEl.style.cssText = 'margin-top:7px';
    idEl = document.createElement('div'); idEl.style.cssText = 'margin-top:7px;padding-top:7px;border-top:1px solid #2a2a2a';
    idEl.addEventListener('mousedown', (e) => e.stopPropagation());
    bodyEl = document.createElement('div'); bodyEl.appendChild(statusEl); bodyEl.appendChild(idEl);
    ...
    applyPanelPosition();
    let drag = null; const DRAG_IGNORE = 'input,textarea,select,button,a,label';
    ...
    const endDrag = () => { if (!drag) return; try { panel.releasePointerCapture(drag.id); } catch (_) {} drag = null; pstore('tw_pos', JSON.stringify({ left: panel.style.left, top: panel.style.top })); };
    panel.addEventListener('pointerup', endDrag); panel.addEventListener('pointercancel', endDrag);
    minimized = pload('tw_min') === '1'; setMinimized(minimized); renderId();
    return panel;
  }
  function renderId() {
    ensurePanel(); if (disabled) return;
    if (MYID) {
      idEl.innerHTML = '<span style="color:#777">Your ID: ' + MYID + '</span> <a href="#" id="tw-id-edit" style="color:#5ba4f5;text-decoration:none">change</a>';
      idEl.querySelector('#tw-id-edit').onclick = (e) => { e.preventDefault(); MYID = ''; pstore('tw_myid', ''); renderId(); poll(); };
    } else {
      idEl.innerHTML = '<div style="color:#f0a832;margin-bottom:4px">⚠ Set your Torn ID for personal limits</div>' +
        '<input id="tw-id-in" placeholder="Your Torn ID" inputmode="numeric" style="width:96px;' + INP + '"> <button id="tw-id-save" style="' + BTN + '">Save</button>';
  ...
  function renderStatus(state, err) {
    ...
    ensurePanel(); if (disabled) return;
    if (err) { setHeadTitle('⚠ ', '#f0a832'); statusEl.innerHTML = 'backend unreachable<br><span style="color:#777">ATTACKS ALLOWED</span>'; return; }
    if (!state || !state.active) { setHeadTitle('✅ ', '#22c48a'); statusEl.innerHTML = 'no active war<br><span style="color:#777">ATTACKS ALLOWED</span>'; return; }
    ...
    const row = (label, val, target) => {
      const t = Number(target) || 0; const cap = t > 0 ? t : '∞';
      const hit = t > 0 && val != null && Number(val) >= t;
      const v = (val == null) ? '–' : val;
      return '<br><span style="color:' + (hit ? hitC : okC) + '">' + label + ': ' + v + ' / ' + cap + '</span>';
    };
    let body = (state.opponent_name ? '<br><span style="color:#bbb">vs ' + esc(state.opponent_name) + '</span>' : '');
    body += row('Agreed score', state.our_score ?? 0, state.total_score_target);
    if (MYID) { body += row('My score', MYSCORE, state.per_member_score_target); body += row('My hits', MYATK, state.max_attacks_per_member); }
    const idleN = Number(state.idle_minutes_target ?? IDLE_TARGET_MIN ?? 0);
    if (ENFORCE_ACTIVITY_RULE) body += idleN > 0 ? '<br><span style="color:#777">Allowed idle attack ≥ ' + idleN + ' min</span>' : '<br><span style="color:#777">No activity restriction</span>';
  ...
  function startRealtime() {
    ...
    function connect() {
      let sock; try { sock = new WebSocket(WSS); } catch (_) { return; }
      sock.onopen = () => {
        backoff = 2000;
        try {
          sock.send(JSON.stringify({ topic: 'realtime:tw', event: 'phx_join', ref: '1', join_ref: '1',
  ...
  async function poll() {
    if (disabled) return;
    try {
      const rows = await gmGet('/rest/v1/war_state?faction_id=eq.' + FACTION_ID + '&select=active,opponent_name,faction_blocked,our_score,total_score_target,per_member_score_target,max_attacks_per_member,idle_minutes_target,war_id');
      const st = Array.isArray(rows) ? rows[0] : null;
      LAST_STATE = st; WAR_ACTIVE = !!(st && st.active);
      MYSCORE = null; MYATK = null; ENEMY_TS_BY_UID = {}; ENEMY_IDS = null;
      ...
      if (st && st.active) {
        ...
        try {
          ...
          if (Array.isArray(es)) { ENEMY_IDS = new Set(); es.forEach((row) => { ENEMY_IDS.add(String(row.member_id)); if (row.last_action_ts) ENEMY_TS_BY_UID[String(row.member_id)] = Number(row.last_action_ts); }); }
        } catch (_) {}
      }
  ...
  // ---- observe AJAX ----
  let applyTimer = null;
  const obs = new MutationObserver(() => { if (applyTimer) return; applyTimer = setTimeout(() => { applyTimer = null; applyAll(document); }, 200); });
  ...
  function route() {
    if (routeActive()) { ensurePanel(); if (disabled || !panel) return; panel.style.display = ''; poll(); }
    else if (panel) { panel.style.display = 'none'; hideTip(); }
  }
  route(); poll();
